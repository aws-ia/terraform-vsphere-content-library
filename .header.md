...
This Terraform module creates or imports a [datastore]-backed standalone [content library][content_library]. The content library may contain new [items][items] or items that already exist in your [VMware Cloud on AWS][vmconaws] or [VMware vSphere][vsphere] on&#8209;premises environment, including deployment media (such as OVA and ISO images), files, and virtual-machine (VM) templates.
